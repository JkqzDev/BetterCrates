...
final class Utils {
    ...
    static public function stringToPosition(string $data): Position {
        ...
        if (!Server::getInstance()->getInstance()->getWorldManager()->isWorldGenerated($data[0])) throw new RuntimeException('World isn\'t generated');
        if (!Server::getInstance()->getInstance()->getWorldManager()->isWorldLoaded($data[0])) Server::getInstance()->getWorldManager()->loadWorld($data[0]);
